...
class CardStreamController {
    constructor() {
        ...
        if (!this.container || !this.cardLine) {
            console.error('Card scanner elements not found!', {
                container: !!this.container,
                cardLine: !!this.cardLine
            });
            return;
        }

        console.log('CardStreamController initializing...');
    ...
    populateCardLine() {
        console.log('Populating card line with 30 cards...');
        ...
        console.log(`Added ${cardsCount} cards to card line. Children count:`, this.cardLine.children.length);
    }
...
class ParticleScanner {
    constructor() {
        ...
        if (!this.canvas) {
            console.warn('Scanner canvas not found');
            return;
        }
...
function initCardScanner() {
    // Wait for React to render the elements, with retry logic
    let attempts = 0;
    ...
    const tryInit = () => {
        ...
        if (cardStream && cardLine && particleCanvas && scannerCanvas) {
            console.log('Card scanner elements found, initializing...');
            ...
            console.log('Card scanner initialized successfully!');
        } else {
            attempts++;
            if (attempts < maxAttempts) {
                console.log(`Card scanner elements not ready, retrying... (${attempts}/${maxAttempts})`);
                setTimeout(tryInit, 200);
            } else {
                console.error('Failed to initialize card scanner: elements not found after max attempts');
            }
        }
    };
