...
export function Iphone15Pro({ children, className, screenClassName }: Iphone15ProProps) {
  return (
    <div
      className={cn(
        "relative mx-auto aspect-[9/16.6] w-full max-w-[320px] rounded-[2.4rem] border border-white/15 bg-zinc-950 p-2 shadow-[0_32px_72px_-42px_rgba(8,119,255,0.7)]",
        className,
      )}
      aria-hidden
    >
      ...
      <div className="pointer-events-none absolute inset-[1px] rounded-[2.25rem] border border-white/10" />
      <div
        className={cn(
          "relative h-full w-full overflow-hidden rounded-[2rem] border border-white/10 bg-black",
          screenClassName,
        )}
      >
        {children}
